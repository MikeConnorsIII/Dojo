...
//filter method on the items
// searchingby is the key of the object that we need
// searching for is the exact term that we need
// people searchFor searchBY

const search = (items, searchFor, searchBy) => {

    return items.filter(object=>{
        let isMatch = true
        for ( let key in searchFor){
            if(object[searchBy][key] != searchFor[key]){
                isMatch = false
...
console.log(search(people, searchFor1, searchBy1))
console.log(search(people, searchFor2, searchBy2))
console.log(search(people, searchFor3, searchBy3))
